...
public class SearchDaoImpl implements SearchDao {
    ...
    private static final String SQL_GET_USERS_COUNT_BY_KEYWORD = "SELECT count(*) FROM short_user LEFT JOIN user_info ON short_user.user_id = user_info.user_id, to_tsquery('russian', :keyword) q WHERE s_user_fts @@ q OR user_fts @@ q;";
    private static final String SQL_GET_THEMES_BY_KEYWORD_SECTION_URL_SUBSECTION_URL_WITH_LIMIT_OFFSET = "SELECT theme.theme_id, theme.user_id, theme.date, messages_count, status, title, nick_name " +
    ...
    private static final String SQL_GET_THEMES_BY_KEYWORD_SECTION_URL_WITH_LIMIT_OFFSET = "SELECT theme.theme_id, theme.user_id, theme.date, messages_count, status, title, nick_name FROM theme INNER JOIN short_user ON short_user.user_id = theme.user_id  LEFT JOIN message ON theme.theme_id = message.theme_id, to_tsquery('russian', :keyword) q WHERE theme_fts @@ q OR message_fts @@ q AND section_id = (SELECT section_id FROM section WHERE LOWER(url) = :urlSection) " +
            "ORDER BY ts_rank_cd(theme_fts, q, 2) DESC, ts_rank_cd(message_fts, q, 2) LIMIT :count OFFSET :offset;";
    private static final String SQL_GET_THEMES_BY_KEYWORD_SUBSECTION_URL_WITH_LIMIT_OFFSET = "SELECT theme.theme_id, theme.user_id, theme.date, messages_count, status, title, nick_name FROM theme INNER JOIN short_user ON short_user.user_id = theme.user_id  LEFT JOIN message ON theme.theme_id = message.theme_id, to_tsquery('russian', :keyword) q WHERE theme_fts @@ q OR message_fts @@ q AND subsection_id = (SELECT subsection_id FROM subsection WHERE LOWER(url) = :urlSubsection) " +
            "ORDER BY ts_rank_cd(theme_fts, q, 2) DESC, ts_rank_cd(message_fts, q, 2) LIMIT :count OFFSET :offset;";
    private static final String SQL_GET_THEMES_WITH_LIMIT_OFFSET = "SELECT theme.theme_id, theme.user_id, date, messages_count, status, title FROM theme, nick_name INNER JOIN short_user ON short_user.user_id = theme.user_id " +
    ...
    private static final String SQL_GET_THEMES_SECTION_URL_WITH_LIMIT_OFFSET = "SELECT theme.theme_id, theme.user_id, date, messages_count, status, title, nick_name FROM theme INNER JOIN short_user ON short_user.user_id = theme.user_id WHERE section_id = (SELECT section_id FROM section WHERE LOWER(url) = :url1) " +
            "ORDER BY theme_id LIMIT :count OFFSET :offset;";
    private static final String SQL_GET_THEMES_SUBSECTION_URL_WITH_LIMIT_OFFSET = "SELECT theme.theme_id, theme.user_id, date, messages_count, status, title, nick_name FROM theme INNER JOIN short_user ON short_user.user_id = theme.user_id WHERE subsection_id = (SELECT subsection_id FROM subsection WHERE LOWER(url) = :url2) " +
            "ORDER BY theme_id LIMIT :count OFFSET :offset;";
    private static final String SQL_GET_THEMES_SECTION_URL_SUBSECTION_URL_WITH_LIMIT_OFFSET = "SELECT theme.theme_id, theme.user_id, date, messages_count, status, title, nick_name " +
